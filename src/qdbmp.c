...
**************************************************************/
BMP* BMP_ReadFile(FILE *log, const char* filename )
{
	BMP*	bmp;
	FILE*	f;
	...
	{
		if (log != NULL){
			fprintf(log, "[ERROR]\t [%s:%d]\t: (BMP_ReadFile)\t: No filename given\n", __FILE__, __LINE__);
		}
		BMP_LAST_ERROR_CODE = BMP_INVALID_ARGUMENT;
		return NULL;
	}
	...
	{
		if (log != NULL){
			fprintf(log, "[ERROR]\t [%s:%d]\t: (BMP_ReadFile)\t: Out of memory\n", __FILE__, __LINE__);
		}
		BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		return NULL;
	}
	...
	{
		if (log != NULL){
			fprintf(log, "[ERROR]\t [%s:%d]\t: (BMP_ReadFile)\t: Cannot open file\n", __FILE__, __LINE__);
		}
	...
	{
		if (log != NULL){
			fprintf(log, "[ERROR]\t [%s:%d]\t: (BMP_ReadFile)\t: Invalid magic bytes\n", __FILE__, __LINE__);
		}
	...
	if ( ( bmp->Header.BitsPerPixel != 32 && bmp->Header.BitsPerPixel != 24 && bmp->Header.BitsPerPixel != 8 )
		|| bmp->Header.CompressionType != 0 
		|| ((bmp->Header.HeaderSize != 40 ) && (bmp->Header.HeaderSize != 56 ) && (bmp->Header.HeaderSize != 108 ) && (bmp->Header.HeaderSize != 124))
		)
	{
		if (log != NULL){
