...
static struct COORDS BROWSER_COORDS(struct config_object cfg){
	...
	coords.x = 0;
	coords.y = 0;
	coords.w = cfg.menu_screen_w - cfg.bmp_w;
	coords.h = cfg.menu_screen_h - (7 * FONT_H);
	return coords;
}
...
static struct COORDS ALPHABET_COORDS(struct config_object cfg){
	COORDS coords;
	coords.x = 0;
	coords.y = cfg.menu_screen_h - (7 * FONT_H);
	coords.w = cfg.menu_screen_w;
	coords.h = FONT_H + 3;	
	return coords;
}
...
static struct COORDS INFO_COORDS(struct config_object cfg){
	COORDS coords;
	coords.x = 0;
	coords.y = cfg.menu_screen_h - (6 * FONT_H) + 3;
	coords.w = cfg.menu_screen_w;
	coords.h = (5 * FONT_H) + 3;	
	return coords;
}
...
static struct COORDS GAMECOVER_COORDS(struct config_object cfg){
	COORDS coords;
	coords.x = cfg.menu_screen_w - (cfg.bmp_w + 2);
	coords.y = 0;
	coords.w = cfg.bmp_w + 2;
	coords.h = cfg.menu_screen_h - (7 * FONT_H);	
	return coords;
}
...
static struct COORDS READER_COORDS(struct config_object cfg){
	...
	coords.x = 20;
	coords.y = 20;
	coords.w = (cfg.menu_screen_w - 40);
	coords.h = (cfg.menu_screen_h - 40);
	return coords;
}
...
static struct COORDS CONFIG_COORDS(struct config_object cfg){
	...
	coords.x = 80;
	coords.y = 20;
	coords.w = (cfg.menu_screen_w - 160);
	coords.h = (cfg.menu_screen_h - 40);	
	return coords;
}
...
int menuGamecoverInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = GAMECOVER_COORDS(menu_cfg);
...
int menuBrowserInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = BROWSER_COORDS(menu_cfg);
	menuBorders(display, log, coords.x, coords.y, coords.w, coords.h, 1, 0);
	return 0;
}
...
int menuCategoryInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = ALPHABET_COORDS(menu_cfg);
	menuBorders(display, log, coords.x, coords.y, coords.w, coords.h, 1, 0);
	return 0;
}
...
int menuInfoInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = INFO_COORDS(menu_cfg);
	menuBorders(display, log, coords.x, coords.y, coords.w, coords.h, 1, 0);
	return 0;
}
...
int menuTextreaderInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = READER_COORDS(menu_cfg);
...
int menuConfigInit(agnostic_bitmap *display, FILE *log){
	...
	COORDS coords = CONFIG_COORDS(menu_cfg);
	menuBorders(display, log, coords.x, coords.y, coords.w, coords.h, 1, 4);
	return 0;
}
...
int menuGamecoverLoad(agnostic_bitmap *display, struct WINDOW_STATE *window_state, FILE *log, char *fname){
	...
	src.window.w = bmp.w;	// Crop to our sizes defined in menu.h
	src.window.h = bmp.h;	// Crop to our sizes defined in menu.h

	// Destination
	dest.window.x = coords.x + 1; //(MENU_SCREEN_W - menu_cfg.bmp_w); 	// Load the bitmap into the bordered window          
	...
	// If the bitmap is narrower than the destination window, then centre it in the window
	if (bmp.w < dest.window.w){
		dest.window.x += (dest.window.w - bmp.w) / 2;
	}
	
	// If the bitmap is shorter than the destination window, then centre it in the window
	if (bmp.h < dest.window.h){
		dest.window.y += (dest.window.h - bmp.h) / 2;	
	}
	
	// Write bitmap to display
	r = gfxBlitBMP(log, &bmp, &src, display, &dest, 0);
	if ( r != 0){
...
int menuCategoryPopulate(agnostic_bitmap *display, FILE *log, struct GAME_DATA *game_data, struct WINDOW_STATE *window_state){
	...
	COORDS coords = ALPHABET_COORDS(menu_cfg);	// Coordinates of window
	//int r = 0;							// return codes
	int i;								// loop counter
...
int menuConfigPopulate(agnostic_bitmap *display, FILE *log, struct GAME_DATA *game_data, struct WINDOW_STATE *window_state){
	...
	COORDS coords = CONFIG_COORDS(menu_cfg);	// Coordinates of window
...
int menuTextreaderPopulate(agnostic_bitmap *display, FILE *log, struct GAME_DATA *game_data, struct WINDOW_STATE *window_state){
	...
	struct COORDS coords = READER_COORDS(menu_cfg);								// Geometry of reader window
...
int menuInfoPopulate(agnostic_bitmap *display, FILE *log, struct GAME_DATA *game_data, struct WINDOW_STATE *window_state){
	...
	int readme_y_pos = coords.y + 2 + (1 * FONT_H);
	int gamedata_y_pos = coords.y + 2 + (2 * FONT_H);
	int gamedata_y_pos2 = coords.y + 2 + (3 * FONT_H);
...
int menuInitWindowstate(FILE *log, struct WINDOW_STATE *window_state){
	...
	COORDS browser_coords = BROWSER_COORDS(menu_cfg);
	window_state->browser_window.max_lines = ((browser_coords.h - 3) / FONT_H) - 1;
...
int main(int argc, char* argv[]){
	...
	r = gfxSetMode(log, &display, menu_cfg.menu_screen_w, menu_cfg.menu_screen_h, MENU_SCREEN_BPP);
	if (r != 0){
	...
	// Fill all windows with content once
