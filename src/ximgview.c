...
static OBJECT rs_ob[] = {
	{-1, 1, 1, G_BOX, OF_NONE, OS_OUTLINED|OS_SHADOWED, (OBJ_INFO)0xFE113CL, 2,1, 23,3},
 	{ 0,-1,-1, G_BOXTEXT, OF_LASTOB, OS_OUTLINED, (OBJ_INFO)&rs_tdi, 2,1, 19,1}
};
...
int main(int argc, char *argv[]){
	...
	while(i < argc || (argc < 2 && (strcpy(path, "*.IMG"), fsel_exinput(path, filename, &button, "(X)IMG-Viewer v1.4")) && button)){
		if(button){
		...
		type = strlen(path) - 4;
		if(type >= 0 && (strcmp(&path[type], ".img") == 0 || strcmp(&path[type], ".IMG") == 0)){
			...
			if(error){
				...
				if(!button){
					sprintf(options, "[1][%s |(%d/%d)|Size: %d x %d |Bitplanes: %d |][ Prev |Quit|Next]", file_name(path), i, argc - 1, img.img_w, img.img_h, img.planes);
...
static int depack_img(char *name, IMG_header *pic){
	...
	int 	byte_repeat, patt_repeat, scan_repeat, error = FALSE;
...
void show_img(IMG_header *pic){
	...
	short mx, my, xx, yy, cen_x, cen_y, redraw = TRUE, pxyarray[8], colors;
	MFDB image, screen;
...
void dither(unsigned short *addr, long size, unsigned short width,
	...
	unsigned short first_plane[MAX_PLANES], *last, *idx, *end, *intensity;
	long offset;
	short count;
